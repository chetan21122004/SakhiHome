...
import { getAreasByRegion, getRegionLabel, type AreaRecord } from "@/lib/areas";
import { homeSection } from "@/lib/siteNav";
import Sparkles from "./Sparkles";

const blobBg = "/assets/blobs/063602423687.jpg";

function AreaCard({ area, index }: { area: AreaRecord; index: number }) {
  const href = `/maid-service-in-${area.slug}`;
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full text-left">
        ...
        <h3 className="font-display text-xl md:text-2xl font-bold text-white md:group-hover:text-accent transition-colors duration-300">
          {area.shortName}
        </h3>
        <p className="mt-2.5 md:mt-3 text-sm text-white/70 md:text-white/60 md:group-hover:text-white/90 transition-colors duration-300 leading-relaxed line-clamp-3">
          {area.description}
        </p>
...
function AreaCluster({
  region,
  heading,
}: {
  region: AreaRecord["region"];
  heading: string;
}) {
  ...
  return (
    <div className="mt-10 md:mt-14">
      <div className="mb-5 md:mb-8 flex flex-col items-center text-center">
        <p className="text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] text-accent">
          {getRegionLabel(region)}
        </p>
        <h3 className="mt-2 font-display text-xl md:text-2xl font-bold text-white">{heading}</h3>
      </div>
  ...
  return (
    <section id="areas" className="relative overflow-hidden py-10 md:py-12 bg-gradient-dark-radial">
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="mx-auto max-w-xl md:max-w-3xl text-center mb-4 md:mb-8">
          ...
          <h2 className="mt-5 md:mt-6 font-display text-3xl sm:text-4xl md:text-6xl font-black text-white text-balance tracking-tight">
            Serving <span className="text-gradient-brand">Hinjewadi</span> & Nanded City
          </h2>
          <p className="mt-4 md:mt-5 text-base sm:text-lg md:text-xl text-white/60 leading-relaxed">
            Hinjewadi IT Park plus a second local belt around Nanded City, Uttam Nagar, Shivane and Dhayari.
          </p>
        </div>

        <AreaCluster region="hinjewadi" heading="Hinjewadi IT Park & nearby" />
        <AreaCluster region="nanded-city" heading="Nanded City, Uttam Nagar & Sinhagad Road" />
